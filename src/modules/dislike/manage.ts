...
export class DislikeManage {
  ...
  constructor(
    ...
    preloadedSnapshotInfo: { latest: string | null; time: number; list: string[]; clients: Record<string, LX.Sync.Dislike.ListInfo> },
    preloadedDislikeRules: LX.Dislike.DislikeRules,
    userName: string,
    maxSnapshotNum: number,
  ) {
    this.snapshotDataManage = new SnapshotDataManage(storage, 'dislike', preloadedSnapshotInfo, userName, maxSnapshotNum)
    this.dislikeDataManage = new DislikeDataManage(this.snapshotDataManage, preloadedDislikeRules)
  }
