...
export class ListManage {
  ...
  constructor(
    ...
    preloadedSnapshotInfo: { latest: string | null; time: number; list: string[]; clients: Record<string, LX.Sync.List.ListInfo> },
    preloadedListData: LX.Sync.List.ListData,
    userName: string,
    maxSnapshotNum: number,
  ) {
    this.snapshotDataManage = new SnapshotDataManage(storage, 'list', preloadedSnapshotInfo, userName, maxSnapshotNum)
    this.listDataManage = new ListDataManage(this.snapshotDataManage, preloadedListData)
  }
